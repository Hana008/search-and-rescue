...
class PathPlanning:
    ...
    # === 新增：把某个栅格标成障碍 ===
    def set_obstacle(self, grid_pos):
        x, y = grid_pos
        if 0 <= x < self.cols and 0 <= y < self.rows:
            self.grid[y][x] = 1  # 1 = obstacle

    # === 新增：从当前世界坐标重新规划 ===
    def replan_from_world(self, current_world_pos):
        """
        从当前世界坐标所在的栅格重新跑完整路线：
        当前格子 -> LOIs -> 返回起点
        （课程里通常 LOI 不多，简单点就全重新来）
        """
        current_grid = self.world_to_grid(current_world_pos[0], current_world_pos[1])
        self.start_grid = current_grid
        self.current_waypoint_idx = 1
        return self.plan()    
    ...
    def replan_from_world(self, world_pos):
        """
        从当前世界坐标重新规划一条完整路径：
        - 把当前 (world_x, world_y) 转成栅格 (x, y)
        - 更新 start_grid
        - 重新调用 plan()
        - 把 current_waypoint_idx 设回 1（下一个 waypoint）
        """
        world_x, world_y = world_pos

        # 1) 世界 → 栅格
        new_start = self.world_to_grid(world_x, world_y)
        print(f"[REPLAN] New start grid from world {world_pos} -> {new_start}")

        # 2) 更新起点
        self.start_grid = new_start

        # 3) 重新规划整条路径
        self.plan()

        # 4) 从路径中的第二个点开始作为 waypoint（第一个是当前格子）
        self.current_waypoint_idx = 1    
