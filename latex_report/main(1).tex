...
\begin{algorithm}[!t]
\caption{LiDAR Initialisation and Sector-wise Distance Monitoring}
\begin{algorithmic}[1]
\REQUIRE LiDAR device name, time step $\Delta t$, number of rays $N$, field of view $FOV$
\STATE $lidar \leftarrow \textbf{GetDevice}(\texttt{"lidar"})$
\STATE \textbf{Enable}($lidar$, $\Delta t$)
\STATE $ranges \leftarrow \textbf{GetRangeImage}(lidar)$  \COMMENT{length $N$}
\STATE Define sector indices: $S_L=[0, \lfloor N/3 \rfloor)$, $S_C=[\lfloor N/3 \rfloor, \lfloor 2N/3 \rfloor)$, $S_R=[\lfloor 2N/3 \rfloor, N)$
\LOOP
    \STATE $ranges \leftarrow \textbf{GetRangeImage}(lidar)$
    \STATE $d_L \leftarrow \min(ranges[i] \mid i \in S_L)$
    \STATE $d_C \leftarrow \min(ranges[i] \mid i \in S_C)$
    \STATE $d_R \leftarrow \min(ranges[i] \mid i \in S_R)$
    \STATE \textbf{Print}(\texttt{"LiDAR L/C/R = "}, $d_L$, \texttt{","}, $d_C$, \texttt{","}, $d_R$)
\ENDLOOP
\end{algorithmic}
\end{algorithm}
